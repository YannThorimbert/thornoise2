...
def RandArrayPrng(c, n, prng): #return rand array with values comprised in [0, n[
    return c*(2*prng.random((n,n)) - 1)

def _set_seeded_condition(seed, l, t, a, n, val, flag, ws):
    #lines (can be optimized, corners don't need to be set here...)
    right = (l+1)%ws[0]
    bottom = (t+1)%ws[1]
    prng = RandomState((seed, l,t,n,flag,0)) #left
    a[0,:] = val*(2*prng.random(n+1) - 1)
    prng = RandomState((seed, right,t,n,flag,0)) #right
    a[n,:] = val*(2*prng.random(n+1) - 1)
    prng = RandomState((seed, l,t,n,flag,1)) #top
    a[:,0] = val*(2*prng.random(n+1) - 1)
    prng = RandomState((seed, l,bottom,n,flag,1)) #bottom
    a[:,n] = val*(2*prng.random(n+1) - 1)
    #corners
    prng = RandomState((seed, l,t,n,flag)) #topleft
    a[0,0] = val*(2*prng.random() - 1)
    prng = RandomState((seed, right,t,n,flag)) #topright
    a[n,0] = val*(2*prng.random() - 1)
    prng = RandomState((seed, l,bottom,n,flag)) #bottomleft
    a[0,n] = val*(2*prng.random() - 1)
    prng = RandomState((seed, right,bottom,n,flag)) #bottomright
    a[n,n] = val*(2*prng.random() - 1)
...
def get_seeded_conditions_d2m1n3(truechunk, k, c):
    ...
    h = c.PARAM_H[k]
    l,t = truechunk
    prng = RandomState((c.SEED, l,t,n,0))
    tabh = RandArrayPrng(h,n+1,prng)
...
def generate_terrain_d2m1n3(chunk, c):
    """Returns an array of heigth values using <chunk> as seed and <p> as parameters.
    """
    hmap = np.zeros((c.S,c.S))
    for k in c.LEVELS:
...
def generate_rect_terrain(chunk, c, width, height):
